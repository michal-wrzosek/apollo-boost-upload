...
export default class DefaultClient<TCache> extends ApolloClient<TCache> {
  constructor(config: PresetConfig = {}) {
    if (config) {
      const diff = Object.keys(config).filter(key => PRESET_CONFIG_KEYS.indexOf(key) === -1);
    ...
    const link = ApolloLink.from([errorLink, requestHandler, uploadLink].filter(x => !!x) as ApolloLink[]);
